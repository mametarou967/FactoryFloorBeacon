...
- [x] BLE スキャンスクリプト作成
- [x] iBeacon パケットのパース処理（UUID・RSSI抽出）
- [x] 通過判定ロジックの実装（タイムアウト方式: 10秒圏外で通過と判定）
- [x] UUID プレフィックスフィルタの実装（無関係なBLEデバイスを除外）
- [x] 通過イベントのローカル保存処理（CSV形式: timestamp, scanner_id, uuid, rssi）
- [x] Pi 5 + ビーコン2個で動作確認
...
- [x] Zero 2 WH へのスキャナーソフトウェア導入
- [ ] ビーコンの UUID 割り当て・事前設定（SSS-825 アプリ使用）
- [ ] `person_map.csv` の作成（UUID と担当者名のマッピング）
- [ ] 2台構成でのフロア移動検知動作確認
- [x] Pi 起動時にスキャナーが自動起動するよう systemd サービスとして設定する
