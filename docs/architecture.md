...
- Raspberry Pi での BLE スキャンには `bleak`（Python）を使用（Step1で採用済み）
...
- 同一人物（UUID）の **2Fイベント → 3Fイベント（または逆）が10分以内** であればフロア移動と判定する
  - イベントのタイムスタンプはタイムアウト方式により「圏外になった時点（EXIT時刻）」を指す
  - したがって判定は「2F EXIT → 3F EXIT の時間差が10分以内」という比較になる
  - 最短移動時間: 約30秒（急ぎ足）。時間窓は調整可能なパラメータとして実装する。
- 例: 2階出入口でのEXIT → 3階出入口でのEXIT（10分以内）＝ 2階→3階への移動
