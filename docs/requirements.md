...
- スキャナー: Raspberry Pi Zero 2 WH（小型・安価・BLE 4.2内蔵）
...
- [ ] **ビーコン設定（RF POWER・送信間隔）**:
  - RF POWER: レベル1（-30dBm）〜レベル8（4dBm）。初期値はレベル7（0dBm）。
  - 送信間隔: 100ms〜10,000ms。初期値は500ms。
  - **推奨初期設定**: RF POWER レベル7（0dBm）・送信間隔 500ms（初期値のまま）でStep1を実施。
    - 500msで通過中に6〜20サンプル程度取得でき、RSSIピーク検出に十分な見込み。
    - 電池寿命は3ヶ月を十分クリアできる見込み（CR2032 × 2）。
    - 隣室・廊下からの誤検知が多い場合は RF POWER を下げて調整する。
    - Step1実測後に最終決定する。
- [ ] **RSSIピーク判定パラメータ**: 通過判定ロジックの閾値（サンプル数・dBm変化量）を要検討。Step1の実機テストで経験的に決定する。
