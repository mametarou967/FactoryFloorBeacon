...
- [x] **ビーコン設定（RF POWER・送信間隔）**: Step1実測で確定。
  - RF POWER: **レベル6（-4dBm）**（ポケット内でも検知可能、検知範囲は数メートル程度）
  - 送信間隔: **500ms**（初期値のまま）
- [x] **通過判定ロジック**: Step1実測でタイムアウト方式に確定。
  - RSSIピーク検出方式は廃止（静置ビーコンのRSSIゆらぎで誤発火するため）
  - **タイムアウト方式を採用**: BEACON_TIMEOUT=10秒（20パケット連続ロスト）で圏外と判定
  - イベントのタイムスタンプはビーコンが圏外になった時点（実通過より最大10秒遅延）
  - MIN_RSSI=-85dBm、MIN_PEAK_RSSI=-80dBm
